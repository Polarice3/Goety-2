...
public class GlacialWall extends AbstractMonolith{
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        ...
        if (!this.level.isClientSide && !this.isEmerging()) {
            ...
            if (pSource != DamageSource.STARVE) {
    ...
    protected SoundEvent getHurtSound(DamageSource p_34154_) {
        if (p_34154_ == DamageSource.STARVE){
    ...
    public void die(DamageSource cause) {
        if (this.level instanceof ServerLevel serverLevel){
            ServerParticleUtil.blockBreakParticles(this.getParticles(), new BlockPos(this.position()), Blocks.SNOW_BLOCK.defaultBlockState(), serverLevel);
            ServerParticleUtil.blockBreakParticles(this.getParticles(), new BlockPos(this.position()).above(), this.getState(), serverLevel);
            ServerParticleUtil.blockBreakParticles(this.getParticles(), new BlockPos(this.position()).above().above(), Blocks.SNOW_BLOCK.defaultBlockState(), serverLevel);
        }
    ...
    public void aiStep() {
        ...
        if (!this.isEmerging()){
            if (!this.isActivate()){
                ...
                float damage = 2.0F;
                if (this.level.getBiome(this.blockPosition()).get().shouldSnowGolemBurn(this.blockPosition())){
                    damage *= 2.0F;
                }
                this.hurt(DamageSource.STARVE, this.level.random.nextInt((int) damage));
            }
