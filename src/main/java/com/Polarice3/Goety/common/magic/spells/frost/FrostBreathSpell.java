                ...
                if (SpellConfig.DragonFrostGriefing.get()) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        if (hitResult.getType() == HitResult.Type.BLOCK) {
                            ...
                            if (worldIn.getBlockState(blockPos) == FrostedIceBlock.meltsInto() && blockstate.canSurvive(worldIn, blockPos) && worldIn.isUnobstructed(blockstate, blockPos, CollisionContext.empty()) && !net.minecraftforge.event.ForgeEventFactory.onBlockPlace(entityLiving, net.minecraftforge.common.util.BlockSnapshot.create(worldIn.dimension(), worldIn, blockPos), net.minecraft.core.Direction.UP)) {
                                worldIn.setBlockAndUpdate(blockPos, blockstate);
                                worldIn.scheduleTick(blockPos, Blocks.FROSTED_ICE, Mth.nextInt(entityLiving.getRandom(), 60, 120));
                            }
                        }
                    }
                }
