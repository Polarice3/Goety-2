...
public class RedMossVegetationFeature extends Feature<NetherForestVegetationConfig> {
    public RedMossVegetationFeature(Codec<NetherForestVegetationConfig> p_66361_) {
        super(p_66361_);
    }
    ...
    public boolean place(FeaturePlaceContext<NetherForestVegetationConfig> p_160068_) {
        ...
        if (!blockstate.is(ModTags.Blocks.RED_MOSS_PLANTABLES)) {
            return false;
        } else {
            int i = blockpos.getY();
            if (i >= worldgenlevel.getMinBuildHeight() + 1 && i + 1 < worldgenlevel.getMaxBuildHeight()) {
                ...
                for(int k = 0; k < netherforestvegetationconfig.spreadWidth * netherforestvegetationconfig.spreadWidth; ++k) {
                    ...
                    if (!blockstate1.isAir() && worldgenlevel.isEmptyBlock(blockpos1) && blockpos1.getY() > worldgenlevel.getMinBuildHeight() && blockstate1.canSurvive(worldgenlevel, blockpos1) && worldgenlevel.getBlockState(blockpos1.below()).is(ModTags.Blocks.RED_MOSS_PLANTABLES)) {
