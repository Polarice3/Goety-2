...
public interface NetherBehaviour {
    ...
    static void changeBiome(LevelAccessor levelAccessor, BlockPos blockPos) {
        if (MobsConfig.ObsidianMonolithBiome.get()) {
            ...
            if (levelAccessor instanceof ServerLevel serverLevel) {
                ...
                Optional<Holder<Biome>> biomeHolder = levelAccessor.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getHolder(Biomes.NETHER_WASTES);
                if (biome.is(BiomeTags.IS_FOREST) || biome.is(Tags.Biomes.IS_SWAMP)) {
                    biomeHolder = levelAccessor.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getHolder(Biomes.CRIMSON_FOREST);
                }
                if (biome.is(Tags.Biomes.IS_SANDY)) {
                    biomeHolder = levelAccessor.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getHolder(Biomes.SOUL_SAND_VALLEY);
                }
                if (biomeHolder.isPresent() && biomeHolder.get().unwrapKey().isPresent()) {
                    if (serverLevel.getChunk(blockPos) instanceof LevelChunk chunk) {
                        sendChangedBiome(chunk,
                                blockPos,
                                biomeHolder.get().unwrapKey().get());
                    }
                }
            }
        }
    ...
    private static void sendChangedBiome(LevelChunk chunk, BlockPos pos, ResourceKey<Biome> biome) {
        ModNetwork.INSTANCE.send(PacketDistributor.TRACKING_CHUNK.with(() -> chunk), new SChangeBiomePacket(pos, biome));
    }
    ...
    private static BiomeResolver makeResolver(LevelAccessor levelAccessor, MutableInt p_262615_, ChunkAccess p_262698_, BoundingBox p_262622_) {
        return (p_262550_, p_262551_, p_262552_, p_262553_) -> {
            ...
            if (p_262622_.isInside(blockPos) && !holder.is(BiomeTags.IS_NETHER)) {
                p_262615_.increment();
                if (holder.is(BiomeTags.IS_FOREST) || holder.is(Tags.Biomes.IS_SWAMP)) {
                    Optional<Holder<Biome>> biomeHolder = levelAccessor.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getHolder(Biomes.CRIMSON_FOREST);
                    if (biomeHolder.isPresent()) {
                        return biomeHolder.get();
                    }
                }
                if (holder.is(Tags.Biomes.IS_SANDY)) {
                    Optional<Holder<Biome>> biomeHolder = levelAccessor.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getHolder(Biomes.SOUL_SAND_VALLEY);
                    if (biomeHolder.isPresent()) {
                        return biomeHolder.get();
                    }
                }
